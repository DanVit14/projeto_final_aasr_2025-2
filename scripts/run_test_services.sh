#!/bin/bash
# Executa test_services.sh dentro do container SMTP (que está na rede aasr_net)
...
echo "A executar testes dentro do container SMTP..."
docker-compose exec -T smtp bash /usr/local/bin/test_services.sh
